...
<xsl:apply-templates/>
  ...
  <xsl:variable name="commentGlobal" select="commentGlobal"/>
   ...
   <xsl:for-each select="ent">
    ...
    <xsl:variable name="dtm" select="dtm"/>
    <xsl:variable name="pro" select="pro"/>
    <xsl:variable name="dbl" select="dbl"/>
    <xsl:variable name="web" select="web"/>
    <xsl:if test='(pro = "sup")'>
    ...
    <li class='cible {$dbl}' onclick='if (altClick==0) {$action}' onmouseover='elementOver(this,{$num})' onmouseout='elementOut(this)' style='width:100%'>

      <div class='caseimg'>
      <table style="padding:0;border:0;margin:0" cellspacing="0"><tr><td style="text-align:center;width:56px;height:32px">
       <u>
      ...
      <div class='nom'>
        <span id="accessMc{$num}" style="display:none">
          <img src='<?php echo $_SESSION["ws"]["cheminImg"]?>menuContext.<?php echo $_SESSION["ws"]["formatExt"]?>' onclick="setTimeout('reloadContext()',20)" style='margin-right:8px' title="<?php echo $_SESSION["ws"]["dia"]["menuContext"]?>"/>
        </span>
        <u>
