...
//
// =======================================================================
// Fonction : XSL d'affichage des dossiers en mode grandes icones
// Nom      : grandes.xsl.php
// Version  : 0.8.2
// Date     : 24/04/09
// =======================================================================
...
<xsl:apply-templates/>
  ...
  <xsl:template match="repGalerie">
   ...
   <ul class='v4' name="dragLayer" id="dragLayer">
   <xsl:for-each select="ent">
    ...
    <span class='caseimg'>
      <u>
       ...
       <xsl:if test='(pro != "url")'>
         ...
         <xsl:otherwise>
          <xsl:if test='(ext != "ico")'>
            <img src='<?php echo $_SESSION["ws"]["cheminIcn"]?>{vis}' title="{$nom}" style="max-width:64px;max-height:64px;"/>
          </xsl:if>
          <xsl:if test='(ext = "ico")'>
       ...
       <xsl:if test='(pro = "url")'>
         <xsl:choose>
           <xsl:when test="contains(vis,'http')">
             <img src='{vis}' title="{$nom}" style="max-width:64px;max-height:64px;"/>
           </xsl:when>
           <xsl:otherwise>
             <img src='{vis}' title="{$nom}" style="width:64px;"/>
           </xsl:otherwise>
        ...
        <xsl:if test='(pro = "sup")'>
          <?php echo $_SESSION["ws"]["dia"]["niveauSup"]?>
              ...
              <xsl:if test="($tai &gt; 0) and ($tai &lt; 1024)">
               <span class='dotsep'> • </span> <xsl:value-of select="number(tai)"/> <?php echo $_SESSION["ws"]["dia"]["octet"]?>
              </xsl:if>
              <xsl:if test="($tai &gt;= 1024) and ($tai &lt; 1048576)">
               <span class='dotsep'> • </span> <xsl:value-of select="format-number(number(tai) div number(1024),'#.00')"/> K<?php echo $_SESSION["ws"]["dia"]["octet"]?>
              </xsl:if>
              <xsl:if test="($tai &gt;= 1048576) and ($tai &lt; 1073741824)">
               <span class='dotsep'> • </span> <xsl:value-of select="format-number(number(tai) div number(1048576),'#.00')"/> M<?php echo $_SESSION["ws"]["dia"]["octet"]?>
              </xsl:if>
              <xsl:if test="($tai &gt;= 1073741824)">
               <span class='dotsep'> • </span> <xsl:value-of select="format-number(number(tai) div number(1073741824),'#.00')"/> G<?php echo $_SESSION["ws"]["dia"]["octet"]?>
              </xsl:if>
